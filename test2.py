...
from Test import connect_getPlate, connect_putPlate, connect_getDevInfo
...
class Ui_MainWindow(object):
    def setupUi(self, MainWindow):
        ...
        pixmap = QtGui.QPixmap(file_path(filename='HikLogo.png'))
        ...
        def deleteN(self):
            if len(self.tableWidget.selectedItems()):
                ...
                if len(rows) == 0:
                    for item in self.tableWidget.selectedItems():
                        rows.append(item.row())
                rr = QtWidgets.QMessageBox.question(self.MainWindow, "Warning",
                                                    "Do you wanna delete all " + str(len(rows)) + " numbers",
                                                    QtWidgets.QMessageBox.Yes | QtWidgets.QMessageBox.Cancel)
                if rr == QtWidgets.QMessageBox.Yes:
                    listP = self.list_p[:]
                    listB = self.list_b[:]
